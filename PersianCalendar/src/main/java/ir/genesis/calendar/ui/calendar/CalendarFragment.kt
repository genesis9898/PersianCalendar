package ir.genesis.calendar.ui.calendar
...
import ir.genesis.calendar.CALENDAR_EVENT_ADD_MODIFY_REQUEST_CODE
import ir.genesis.calendar.LAST_CHOSEN_TAB_KEY
import ir.genesis.calendar.PREF_HOLIDAY_TYPES
import ir.genesis.calendar.R
import ir.genesis.calendar.databinding.EventsTabContentBinding
import ir.genesis.calendar.databinding.FragmentCalendarBinding
import ir.genesis.calendar.databinding.OwghatTabContentBinding
import ir.genesis.calendar.entities.CalendarEvent
import ir.genesis.calendar.entities.DeviceCalendarEvent
import ir.genesis.calendar.ui.MainActivity
import ir.genesis.calendar.ui.calendar.dialogs.MonthOverviewDialog
import ir.genesis.calendar.ui.calendar.dialogs.SelectDayDialog
import ir.genesis.calendar.ui.calendar.dialogs.ShiftWorkDialog
import ir.genesis.calendar.ui.calendar.times.TimeItemAdapter
import ir.genesis.calendar.ui.shared.CalendarsView
...
class CalendarFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        bgg.setBackgroundResource(run { var season = (getTodayOfCalendar(CalendarType.SHAMSI).month - 1) / 3

            // Southern hemisphere
            if ((getCoordinate(mainActivity)?.latitude ?: 1.0) < .0) season = (season + 2) % 4

            when (season) {
                0 -> R.drawable.springbg
                1 -> R.drawable.summerbg
                2 -> R.drawable.fallbg
                else -> R.drawable.winterbg
            }
        })
        val tabs = listOf(
        ...
        var lastTab = mainActivity.appPrefs.getInt(LAST_CHOSEN_TAB_KEY, EVENTS_TAB)
        if (lastTab >= tabs.size) lastTab = EVENTS_TAB
